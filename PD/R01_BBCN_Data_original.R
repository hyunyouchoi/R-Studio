...
setwd("C:/Users/ic07949/Desktop/KPMG/Model Development/development code and data/PD/Dataset/BBCN")
#BBCN_df <- read.csv("./Data Input/BBCN/data request bottom-up.csv")
...
setwd("C:/Users/ic07949/Desktop/KPMG/Model Development/development code and data/PD/KPMG")
#rates <- fread("./Data Input/MacroVar/rates2.csv")
...
## !! SAS File 01, Line 126 to 181 can be ignored because it doesn't affect the final results.
## Line 183 to 211 will be coded later.
...
#### 20184 obs in indx_bbcn in R. 20190 obs in indx_bbcn in SAS because SAS includes 6 missing "min_non_acc_date"s
# BBCN_df <- BBCN_df[-grep("non_acc_date", colnames(BBCN_df))] 
BBCN_df <- merge(x = BBCN_df, y = indx_bbcn, by = "accountNo", all.x = TRUE) 


## Clean up data (SAS File 01, Line 243 to 252)
...
BBCN_df <- filter(BBCN_df, maturityDate > 2006 & yr_maturity >= yr_file )
BBCN_df <- filter(BBCN_df, !(yr_maturity == yr_file & (mn_file - mn_maturity)>2) )
BBCN_df <- filter(BBCN_df, currentNetBookBal != 0 )
#### 414335 obs.
...
## delete portfolio_id == "error". (SAS File 01, Line 334 to 337)
BBCN_df <- filter(BBCN_df, portfolio_id != "error")


## delete the observations after the default date (SAS File 01, Line 340 to 344)
...
setwd("C:/Users/ic07949/Desktop/KPMG/Model Development/development code and data/PD/Dataset/output")
save(df_final_bbcn, file = "df_final_bbcn.RData")
write.csv(df_final_bbcn, file = "df_final_bbcn.csv", row.names = FALSE)
