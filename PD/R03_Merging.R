...
df_final_bbcn <- read.csv("df_final_bbcn.csv")
df_final_wilshire <- read.csv("df_final_wilshire.csv")
df_final_wilshire <- df_final_wilshire[1:34]
...
####################################################################################################
## Filter the data  excluded for compare the property_res excluding with all dataset
# df_boh = filter(df_boh, !(current_balance == 0 & y ==0))
####################################################################################################
...
# 
# df_boh_9Q$POB <- ifelse(df_boh_9Q$term_q==0,100,df_boh_9Q$POB)
...
for(scenario in c("base", "adverse", "severe")){
  ...
  macro_var <- fread(paste0(scenario, ".csv"))
...
## Save data 
save(df_boh_base, file = "./Data output/df_boh_base.RData")
save(df_boh_adverse, file = "./Data output/df_boh_adverse.RData")
save(df_boh_severe, file = "./Data output/df_boh_severe.RData")
write.csv(df_boh_base, file = "./Data output/df_boh_base.csv", row.names = FALSE)
write.csv(df_boh_adverse, file = "./Data output/df_boh_adverse.csv", row.names = FALSE)
write.csv(df_boh_severe, file = "./Data output/df_boh_severe.csv", row.names = FALSE)
