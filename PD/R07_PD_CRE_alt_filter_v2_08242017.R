...
setwd("C:/Users/ic07949/Desktop/KPMG/Model Development/development code and data/PD/Dataset/KPMG")
...
## load data
cre_dev <- readRDS("df_boh_base_v2.RDS")
...
## load data
cre_dev1 <- readRDS("df_boh_base.RDS")
cre_dev1$fileDate <- as.Date(cre_dev1$fileDate, "%Y-%m-%d")

# CRE and Date filter
cre_dev1 <- filter(cre_dev1, portfolio_id == "CRE" & as.Date(fileDate) <= as.Date("2016-03-31") & as.Date(fileDate) >= as.Date("2007-12-31") )

# Getting description of the current sample
y_filter_1 <- describe(cre_dev1$y)
bal_sum_1 <- sum(cre_dev1$current_balance)
...
cre_dev1$property_type <- ifelse(cre_dev1$callReportCodeDescr %in% c("Conv 5+ Residential Prop","CONVENTIONAL 5+ RESIDENTIAL"),10,cre_dev1$property_type)

cre_dev1$prop_res <- ifelse( (cre_dev1$boh_id == "bbcn" & cre_dev1$property_type %in% c(10 , 11) ) |
                              (cre_dev1$boh_id == "wilshire" & cre_dev1$property_type %in% c(11 , 12) )  ,
                            1,
                            0)

# Missing Wilshire Property type filter
cre_dev1 <- filter(cre_dev1, !(boh_id == "wilshire" & is.na(property_type)))

## (2a) variable prop_retail: Retail shopping center
##     BBCN: 15, 16, 17, 18
##     wilshire: 1
cre_dev1$prop_retail <- ifelse( (cre_dev1$boh_id == "bbcn" & cre_dev1$property_type %in% c(15, 16, 17, 18) ) |
                                 (cre_dev1$boh_id == "wilshire" & cre_dev1$property_type == 1 )  ,
                               1,
                               0)
## (2b) variable prop_auto: Gas Stations, Car Washes, and Auto Repair Centers
##     BBCN: 33, 34, 36
##     wilshire: 7, 8, 16
cre_dev1$prop_auto <- ifelse( (cre_dev1$boh_id == "bbcn" & cre_dev1$property_type %in% c(33, 34, 36) ) |
                               (cre_dev1$boh_id == "wilshire" & cre_dev1$property_type %in% c(7, 8, 16) )  ,
                             1,
                             0)
...
##     BBCN: 28,29
##     wilshire: 5
cre_dev1$prop_hotel <- ifelse( (cre_dev1$boh_id == "bbcn" & cre_dev1$property_type %in% c(28, 29) ) |
                                (cre_dev1$boh_id == "wilshire" & cre_dev1$property_type == 5 )  ,
                              1,
                              0)

## (3) variable boh_rating1
cre_dev1$boh_rating1 <- ifelse(cre_dev1$boh_rating %in% c(0,1,2,3), "R1",
                              ifelse(cre_dev1$boh_rating %in% c(4,1000), "R2",
                                     ifelse(cre_dev1$boh_rating %in% c(2000,3000), "R3", "error")  
                              ))
#R1 and R2 variables
cre_dev1$boh_rating1_R1 <- ifelse(cre_dev1$boh_rating1 == "R1",1, 0)
cre_dev1$boh_rating1_R2 <- ifelse(cre_dev1$boh_rating1 == "R2",1, 0)

#Lagged R1 and R2
cre_dev1 <- cre_dev1 %>% group_by(account_id) %>% mutate(boh_rating1_R1_l = lag(boh_rating1_R1)) %>% as.data.frame()
cre_dev1 <- cre_dev1 %>% group_by(account_id) %>% mutate(boh_rating1_R2_l = lag(boh_rating1_R2)) %>% as.data.frame()

# Update lagged R1 and R2 values with current value
cre_dev1$boh_rating1_R1_l <- ifelse(is.na(cre_dev1$boh_rating1_R1_l),cre_dev1$boh_rating1_R1,cre_dev1$boh_rating1_R1_l)
cre_dev1$boh_rating1_R2_l <- ifelse(is.na(cre_dev1$boh_rating1_R2_l),cre_dev1$boh_rating1_R2,cre_dev1$boh_rating1_R2_l)

## (4) variable rgdp_qg_lag_2_neg
cre_dev1$rgdp_qg_lag_2_neg <- ifelse(cre_dev1$rgdp_qg_lag_2 >= 0, 0, cre_dev1$rgdp_qg_lag_2 )

## (5) variable CAUR_yd_3
cre_dev1$CAUR_yd_3 <- ifelse(cre_dev1$CAUR_yd >= 3, 3, cre_dev1$CAUR_yd)

## (6) variable CAHPI_ag_6
cre_dev1$CAHPI_ag_6 <- ifelse(cre_dev1$CAHPI_ag >= 6, 6, cre_dev1$CAHPI_ag)

## (7) variable POB_95
cre_dev1$POB_95 <- ifelse(cre_dev1$POB <= 95, 95, cre_dev1$POB)

## (8) Wilshire Dummy
cre_dev1$wilshire_d <- ifelse(cre_dev1$boh_id == "wilshire",1,0)

# Getting description of the current sample
y_filter_2 <- describe(cre_dev1$y)
bal_sum_2 <- sum(cre_dev1$current_balance)

# Balance filter
cre_dev1 <- filter(cre_dev1, !(current_balance == 0 & y == 0))

# Getting description of the current sample
y_filter_3 <- describe(cre_dev1$y)
bal_sum_3 <- sum(cre_dev1$current_balance)

# Number of default events per period
def_events_df <- cre_dev1[,which(colnames(cre_dev1) %in% c("fileDate","y")),drop=F]
def_events_df_sum <- def_events_df %>% group_by(fileDate) %>% summarise(Defaults = sum(y)) %>% data.frame()
Obs <- def_events_df %>% group_by(fileDate) %>% tally() %>% data.frame()
def_events_df_sum <- merge(def_events_df_sum, Obs)
colnames(def_events_df_sum) <- c("Date","No. of Defaults","Observations")
...
# Sample fraction
sample_fraction <- .8
cre_dev_training <- cre_dev1 %>% sample_frac(sample_fraction)
...
##Out of sample data
cre_dev_testing <- cre_dev1[-which(rownames(cre_dev1) %in% rownames(cre_dev_training)),]
...
dep_var_filter_stats <- cbind(dep_var_filter_stats,bal_sum_stats)
rownames(dep_var_filter_stats) <- c("CRE & (2007 Q4 - 2016 Q1)","Wilshire Property Type","Balance Filter","80% In-sample","20% Out-of-sample")
colnames(dep_var_filter_stats)[8] <- "Sum of Curr. Bal."
dep_var_filter_stats
...
# Main Model
model1 <- y ~ boh_rating1_R1_l + boh_rating1_R2_l + POB_95 + CAUR_yd + NCREIF_Property_Index_ag_lag_1 + prop_res
fit1 <- glm(model1, family = binomial(link = "logit"), data = cre_dev_training)
summary(fit1)

# Multicollinearity of fitted model1
vif(fit1)

# model1 regressed on full sample
fit_full <- glm(model1, family = binomial(link = "logit"), data = cre_dev1)
summary(fit_full)

# McFadden's pseudo R squared for the main model1
pR2 <- 1 - fit1$deviance / fit1$null.deviance 
pR2

###################################
# Output the main regression table in academic format
stargazer::stargazer(fit1,fit_full, type = "text", model.numbers = F, column.labels = c("Train","Full","Wilshire"))

# Save the model coefficients
coef_cre <- as.data.frame(summary(fit1)$coefficients)
...
# Out-of-sample #
## Out-sample prediction p_hat for each account
predict_out <- predict(fit1, cre_dev_testing, type="response")
cre_dev_testing$p_hat <- predict_out
...
cre_pd_training_plot1 <- ggplot(cre_pd_quarterly_in, aes(x=fileDate, y = value, color=variable)) + 
  geom_line() + scale_colour_manual(values=cbPalette) + xlab("Date") + ylab("Default Rate") + ggtitle("Ave. Default Rate CRE - In-sample") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size=15)) + theme(legend.title=element_blank()) + theme(legend.position="bottom")
cre_pd_training_plot1
...
cre_pd_out_plot1 <- ggplot(cre_pd_quarterly_out, aes(x=fileDate, y = value, color=variable)) + 
  geom_line() + scale_colour_manual(values=cbPalette) + xlab("Date") + ylab("Default Rate") + ggtitle("Ave. Default Rate CRE - Out-of-sample") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size=15)) + theme(legend.title=element_blank()) + theme(legend.position="bottom")
cre_pd_out_plot1
...
for(scenario in c("base", "adverse", "severe")){
  ...
  cre_forecast <- readRDS(paste0("df_boh_",scenario, ".RDS"))
  cre_forecast$fileDate <- as.Date(cre_forecast$fileDate, "%Y-%m-%d")
  cre_forecast <- filter(cre_forecast, portfolio_id == "CRE" & as.Date(fileDate) > as.Date("2016-03-31") )
...
## Forecast plot
cbPalette <- c("#000000", "#0072B2", "#006600", "#E69F00", "#D55E00")
cre_pd_plot1 <- ggplot(cre_pd_quarterly_all, aes(x = fileDate, y = PD, color = scenario)) + 
  geom_line() + scale_colour_manual(values=cbPalette) + 
  ggtitle("BOH CRE PD") + xlab("Date") + ylab("Default Rate") + ggtitle("Average Default Rate CRE") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(size=15)) + theme(legend.title=element_blank()) + theme(legend.position="bottom")
...
# Make the summary stats table between the samples
cre_dev_mean <- apply(cre_dev1[,which(colnames(cre_dev1) %in% sum_nms),drop=F],2,function (x) round(mean(x),4))
cre_dev_in_mean <- apply(training_df[,which(colnames(training_df) %in% sum_nms),drop=F],2,function (x) round(mean(x),4))
cre_dev_out_mean <- apply(testing_df[,which(colnames(testing_df) %in% sum_nms),drop=F],2,function (x) round(mean(x),4))

cre_dev_sd <- apply(cre_dev1[,which(colnames(cre_dev1) %in% sum_nms),drop=F],2,function (x) round(sd(x),4))
cre_dev_in_sd <- apply(training_df[,which(colnames(training_df) %in% sum_nms),drop=F],2,function (x) round(sd(x),4))
cre_dev_out_sd <- apply(testing_df[,which(colnames(testing_df) %in% sum_nms),drop=F],2,function (x) round(sd(x),4))

cre_dev_max <- apply(cre_dev1[,which(colnames(cre_dev1) %in% sum_nms),drop=F],2,function (x) round(max(x),4))
cre_dev_in_max <- apply(training_df[,which(colnames(training_df) %in% sum_nms),drop=F],2,function (x) round(max(x),4))
cre_dev_out_max <- apply(testing_df[,which(colnames(testing_df) %in% sum_nms),drop=F],2,function (x) round(max(x),4))

cre_dev_min <- apply(cre_dev1[,which(colnames(cre_dev1) %in% sum_nms),drop=F],2,function (x) round(min(x),4))
cre_dev_in_min <- apply(training_df[,which(colnames(training_df) %in% sum_nms),drop=F],2,function (x) round(min(x),4))
cre_dev_out_min <- apply(testing_df[,which(colnames(testing_df) %in% sum_nms),drop=F],2,function (x) round(min(x),4))

cre_dev_n <- apply(cre_dev1[,which(colnames(cre_dev1) %in% sum_nms),drop=F],2, length)
cre_dev_in_n <- apply(training_df[,which(colnames(training_df) %in% sum_nms),drop=F],2, length)
cre_dev_out_n <- apply(testing_df[,which(colnames(testing_df) %in% sum_nms),drop=F],2, length)
...
set.seed(20170808)
cre_dev_training_s <- cre_dev1 %>% sample_frac(sample_fraction)
cre_dev_testing_s <- cre_dev1[-which(rownames(cre_dev1) %in% rownames(cre_dev_training_s)),]
...
for (i in 1:sample_number){
  ##In sample data
  set.seed(i)
  df_sample_in <- cre_dev1 %>% sample_frac(sample_fraction)
  ##Out of sample data
  df_sample_out <- cre_dev1[-which(rownames(cre_dev1) %in% rownames(df_sample_in)),]
  ##Estimate the model
  logit_s <- glm(model1, family = binomial(link = "logit"), data = df_sample_in)
  predict_s[[i]] <- predict(logit_s, df_sample_out, type="response")
  predict_date_s[[i]] <- df_sample_out$fileDate
  s_name[[i]] <- rep(paste("Sample",i,sep="_"),nrow(df_sample_out))
}
...
# Plot of all Forecasts 
sample_fcst_p1 <- ggplot(data = Prediction_df_gg, aes(x = Date, y = value, group = Sample, color = Sample)) + geom_line() + xlab("Date") + ylab("Default Rate") + ggtitle("Ave. Default Rate CRE Out-of-Sample") + theme(plot.title = element_text(hjust = 0.5)) +  theme(text = element_text(size=12)) + theme(legend.title=element_blank())
...
# Round the sample size to a whole number
# Uses the sample fraction set above to partition in-out samples
sample_size <- round(nrow(cre_dev1)*sample_fraction)
...
for (i in 1:sample_number){
  ##In sample data
  set.seed(i)
  df_sample <- cre_dev1[sample(nrow(cre_dev1), sample_size, replace = FALSE), ]
  logit <- glm(model1, family = binomial(link = "logit"), data = df_sample)
  coeff_l[[i]] <- round(coef(summary(logit))[,1],5)
  pval_l[[i]] <- round(coef(summary(logit))[,4],5)
}
end.time <- Sys.time()
